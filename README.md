...
| RAG Agent Chatbot for Document Interaction | A comprehensive RAG workflow with chatbot functionality that supports multiple document types (PDF, DOCX, TXT, MD). Features document ingestion, vector database storage with Qdrant, semantic search, conversation memory, and source citations. Enables natural language Q&A across all uploaded documents. | AI Research, Document Processing | [RAG Agent Chatbot for Document Interaction](./AI_Research_RAG_and_Data_Analysis/RAG%20Agent%20Chatbot%20for%20Document%20Interaction.json) |
